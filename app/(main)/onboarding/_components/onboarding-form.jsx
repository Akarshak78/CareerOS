...
const OnboardingForm = ({ industries, initialData }) => {
  ...
  useEffect(() => {
    ...
    const matchedIndustry = industries.find((ind) =>
      initialData.industry?.startsWith(`${ind.id}-`)
    );

    if (matchedIndustry) {
      const subSlug = initialData.industry.slice(matchedIndustry.id.length + 1);
      const matchedSub = matchedIndustry.subIndustries.find(
        (sub) => sub.toLowerCase().replace(/ /g, "-") === subSlug
      );
      setSelectedIndustry(matchedIndustry);
      setValue("industry", matchedIndustry.id);
      if (matchedSub) setValue("subIndustry", matchedSub);
    }
  ...
  useEffect(() => {
    if (updateResult?.success && !updateLoading) {
      toast.success(
        initialData ? "Profile updated successfully!" : "Profile completed successfully!"
      );
    ...
    <div className="flex items-center justify-center bg-background">
      <Card className="w-full max-w-lg mt-10 mx-2">
        <CardHeader>
          ...
          <CardDescription>
            {initialData
              ? "Update your industry, skills, and experience anytime."
